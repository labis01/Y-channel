...
1b. **Two independent drivers, not one — from a 7-video transcript comparison**, Sept 2026: the
   outlier ("Could L Catch Spider-Man?", 394K), #2 ("Could L Catch Dexter Morgan?", 240K), #3
   ("Could Batman Catch Spider-Man?", 111K), #4 ("Could L Catch Batman?", 71K), #5 ("Could L
   Outsmart Jigsaw?", 46K), #6 ("Could Dexter Catch Light Yagami?", 37K), and the weakest video
   ("Could L Catch The Joker?", 20K) — 7 of Specular Verse's 8 videos now read. The two factors
   below turned out to be **separable and additive**, not one and the same:

   | | Confident tense | Hedged tense |
   |---|---|---|
   | **Strong dual-agency** | Spider-Man 394K, Dexter Morgan 240K, Batman-vs-Spider-Man 111K (avg ~248K) | Batman 71K, Jigsaw 46K, Dexter/Light 37K (avg ~51K) |
   | **Weak dual-agency** | — | Joker 20K |

   The confident-tense quadrant now has **three** data points too (adding "Could Batman Catch
   Spider-Man?", written in flat present tense throughout with a genuinely sustained mutual
   chess match — Peter builds a mechanical decoy dummy, constantly adapts routes; Batman adapts
   right back) and it lands at 111K, third-highest of all eight videos, confirming the top
   quadrant rather than the two-video sample being a fluke. With 7 of 8 videos now sampled,
   confident-tense videos average **~5x** the views of hedged-tense videos with comparably strong
   dual-agency, holding steady as more data comes in. Treat hedged tense as close to a hard rule
   to avoid for every future script, not just a leaning.
...
- Case File 004: Could L Catch Vought? (Vought International, the fictional conglomerate from
  ...
  **Real performance (via user report, exact timeline unconfirmed — first stated as ~2 weeks
  post-publish, then as 48 hours; not resolved, no Studio screenshot provided)**: **3 views
  total, ~0 watch hours.** Whatever the exact window, this is at or below Case File 003's already
  -weak baseline (8 views in its first 3 days 17 hours) — not an improvement. Matches the
  distribution risk flagged at topic-selection time: Vought is a corporate entity, not a single
  recognizable face, a structural departure from every one of Specular Verse's proof-point
  videos. Can't fully separate "topic lacks a searchable single name" from "reach is broken
  channel-wide" without CTR/impression data (not shared). Treat as a second weak-performance data
  point reinforcing **recognizable individual character** as close to a hard requirement for
...
- **First concrete production step**: generate the Dani anchor image (prompt in the character
  ...
  silent (no dialogue), but Grok generated an ad-libbed line anyway, so her voice locked there
  instead of on 1B as originally planned. Check every generated beat for audio, even ones marked
  silent — whatever voice appears first is the one to lock and reuse, same "actual output is the
  source of truth" rule as Logic Nexus's recorded narration. Once her face (from the anchor) and
  voice (from whichever beat first produced it) are locked, the rest of the cast follows the same
  pattern.
...
- **Animation (revised, locked) — two-phase, stills first, then video for every beat**: reversed
  twice. First reversed from stills-only after checking Grace Studios (reference channel) —
  "Made with AI," image/video + narration, "a similar toolkit to Logic Nexus's, just scaled up" —
  landing on "mostly stills, 3 real-video beats." Then, after diagnosing 1A's problems (key/lock
  misplacement, ad-libbed voice, door drift into 2A) as coming from generating video directly
  from a prompt with no still checkpoint, the fix (lock a still first, then convert to video) was
  generalized to every beat — and from there the user decided **every one of the ~55 beats
  eventually becomes video**, not just 3, closer to the original "this is a movie, not a
  slideshow" intent, now made safer by the stills-first checkpoint.
  **Locked two-phase workflow**: Phase 1 — generate and lock all ~55 stills (Image to Image),
  each checked against location/character/object references before moving on, same as before.
  Phase 2 — once the full still set is locked and consistent, go back through and convert each
  still to video (Image to Video / Frame to Video in Grok), seeded from the already-verified
  still rather than generated fresh from a prompt. Grok caps each video generation at 15 seconds
  — for a beat needing more, chain clips using the previous clip's last frame as the next
  reference image, then stitch as one continuous shot in CapCut. **Cost tradeoff, flagged
  explicitly**: this roughly doubles total Grok generations (~55 stills + up to ~55 video
  conversions, vs. the earlier ~58 total under "3 real-video beats") — a real credit cost, but
  each conversion starts from a locked, verified still, so it shouldn't repeat 1A's failures.
